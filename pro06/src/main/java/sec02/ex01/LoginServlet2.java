...
/*
login2.html(아이디와 비밀번호를 입력해서 로그인버튼을 눌러 서블릿을 요청하는 화면)에서 
...
순서2. 웹서버 (아파치)는  login2.html파일을 찾아 실행한 HTML코드 디자인을 
           클라이언트의 웹브라우저 화면에 표시 해 줍니다.

순서3. login2.html화면을 본~ 클라이언트는 아이디와 비밀번호를 입력하고  
...
login2.html(아이디와 비밀번호를 입력해서 로그인버튼을 눌러 서블릿을 요청하는 화면)에서 
로그인 버튼을 눌러  http://localhost:8181/pro06/login2 
요청주소를 톰캣 서버에 전달하여 요청하면
요청을 받는 LoginServlet클래스 입니다.
*/
@WebServlet("/login2")
