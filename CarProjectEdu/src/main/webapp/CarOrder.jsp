...
<div class="container">	
	...
	<form action="<%=contextPath%>/Car/CarOrder.do" method="post">
				...
				<dl class="detail-list">			
					...
					<div class="detail-row">
						...
						<dd>						
							...
							<%-- 조건 ${requestScope.vo.carins == 1} 이 맞을 때만(자차 보험 적용 <option value="1">선택 했을때만) 아래를 화면에 그린다 --%>							
							<c:if test="${requestScope.vo.carins == 1}">							
								<span class="badge badge-brand">자차보험</span>
								<c:set var="hasOption" value="true"/>
							</c:if>
							<%-- 조건 ${requestScope.vo.carwifi == 1} 이 맞을 때만(무선 WiFi 적용 <option value="1">선택 했을때만) 아래를 화면에 그린다 --%>
							<c:if test="${requestScope.vo.carwifi == 1}">
								<span class="badge badge-brand">무선 WiFi</span>
								<c:set var="hasOption" value="true"/>
							</c:if>
							<%-- 조건 ${requestScope.vo.carnave == 1} 이 맞을 때만(네비게이션 적용 <option value="1">선택 했을때만) 아래를 화면에 그린다 --%>
							<c:if test="${requestScope.vo.carnave == 1}">
								<span class="badge badge-brand">네비게이션</span>
								<c:set var="hasOption" value="true"/>
							</c:if>
							<%-- 조건 ${requestScope.vo.carbabyseat == 1} 이 맞을 때만(베이비시트 적용 <option value="1">선택 했을때만) 아래를 화면에 그린다 --%>
							...
							자차 보험  미적용  <option value="0"> 선택 했을때만
							무선 WiFi 미적용  <option value="0"> 선택 했을때만
							네비게이션  미적용  <option value="0"> 선택 했을때만
							베이비시트  미적용  <option value="0"> 선택 했을때만  .... 미적용 !!!! 일떄~~~~ 
							--%>
		...
		<%--
			CarController에서 request 내장객체 바인딩 했었던 모습 
			==================================================
