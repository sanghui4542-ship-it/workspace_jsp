...
<%--
 ============================================================================
  ...
  버튼별 동작 (아래 script 영역)
    등록 : CarApp.postForm 으로 /Board/writePro.bo 에 POST 요청 (화면 이동 없음)
    목록 : 보고 있던 페이지 번호(nowPage, nowBlock)를 그대로 붙여 목록 화면으로 이동

  [예전에 있던 버그 - 여는 <form> 태그가 없었다]
    예전 화면에는 </form> 닫는 태그만 있고 여는 <form> 태그가 없었다.
    jQuery 로 문서 전체에서 입력칸을 찾던 시절에는($("input[name=writer]"))
    폼이 없어도 우연히 동작했지만, 순수 자바스크립트로 바꾸면서 폼 기준으로
    입력칸을 찾게 되어 "폼을 못 찾아 등록 버튼이 동작하지 않는" 문제가
    있었다. 지금은 <form id="writeForm"> 을 제대로 열어 두었다.
    onsubmit="return false;" 를 둔 이유는, 등록을 자바스크립트(AJAX)로
    직접 처리하므로 폼이 브라우저 방식으로 실제 전송(새로고침)되면 안
    되기 때문이다. 이렇게 해 두면 입력칸에서 Enter 를 눌러도 새로고침되지 않는다.
 ============================================================================
--%>
