...
public class CarDAO {
	...
	*/
	...
	/* [정리] 이 자리에 있던 updateOrderPassword() 를 지웠다.
	   예약 비밀번호를 해시로 바꿔 저장하기 위해 만들어 두었던 메소드인데,
	   프로젝트 전체에서 이 메소드를 부르는 곳이 한 군데도 없었다.
	   지금은 CarService 가 비밀번호를 해시 없이 그대로 비교하는 방식이라
	   이 메소드가 쓰일 일이 없다. 나중에 해시 저장으로 바꾼다면
	   "조회하며 비밀번호를 맞춘 그 순간에 해시로 교체" 하는 용도로
	   다시 만들면 된다. */
