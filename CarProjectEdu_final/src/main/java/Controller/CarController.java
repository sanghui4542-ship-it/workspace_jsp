...
/*
 * ============================================================================
 *  CarController  -  사장(Controller) : 렌터카 예약 관련 요청을 가장 먼저 받는 서블릿
 *
 *  이 사장이 받는 요청 주소 (2단계 주소)
 *
 *    [차량 둘러보기]
 *      /Main                 메인 화면 (Center.jsp + 전체 차량 목록)
 *      /bb                   예약하기 메뉴 -> 검색 화면(CarReservation.jsp)
 *      /CarList.do           전체 차량 목록 조회
 *      /carcategory.do       등급별(Small/Mid/Big) 차량 목록 조회
 *      /CarInfo.do           차량 1대 상세 조회
 *
 *    [예약하기]
 *      /CarOption.do         추가 옵션 선택 화면
 *      /CarOptionResult.do   옵션 포함 총 금액 계산 (AJAX)
 *      /CarOrder.do          결제 후 예약 등록
 *
 *    [예약 확인 / 변경 / 취소]
 *      /cc                   예약 확인 입력 화면(CarReserveConfirm.jsp)
 *      /CarReserveConfirm.do 연락처 + 비밀번호로 예약 조회
 *      /update.do            예약 수정 화면
 *      /updatePro.do         예약 수정 실행
 *      /delete.do            예약 취소 입력 화면(Delete.jsp)
 *      /deletePro.do         예약 취소 실행
 *
 *    [챗봇이 부르는 주소 - 응답이 JSON 이다]
 *      /orderListJson.do     내 예약 목록 조회
 *      /orderCancelJson.do   예약 취소
 *
 *    [기타]
 *      /ai                   AI 추천 서비스 화면(AIService.jsp)
 *      /NaverSearchAPI.do    네이버 블로그 검색 (외부 API 호출)
 *
 *  [화면 이동 방식]
 *    이 프로젝트의 모든 화면은 CarMain.jsp 한 장 안에서 가운데만 바뀐다.
 *    그래서 대부분의 분기는 아래 두 가지를 request 에 담고 /CarMain.jsp 로
 *    포워딩하는 것으로 끝난다.
 *        center : 가운데에 끼워 넣을 화면 주소 (예: "CarList.jsp")
 *        그 화면이 쓸 조회 결과 (예: carList, vo)
 *    반대로 AJAX 요청(/CarOptionResult.do 등)과 JSON 요청은 화면으로
 *    포워딩하지 않고 out.write() 로 글자만 응답하고 곧바로 끝낸다.
 * ============================================================================
 */
...
public class CarController extends BaseController {
	...
	*/
	...
	protected void doHandle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// (받을 수 있는 2단계 주소 전체 목록은 이 클래스 맨 위 설명을 참고)
